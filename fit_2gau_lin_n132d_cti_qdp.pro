...
pro fit_2gau_lin_n132d_cti_qdp, file_counts_qdp, emin, emax, centroid_guess1, centroid_guess2, key_plot = key_plot
  ...
  if n_params() lt 5 then begin
     print,"Please enter correct parameters: fit_2gau_lin_n132d_cti_qdp,filename,en_min, en_max, centroid_guess1, centroid_guess2, key_plot = 1"
     print,"Please enter correct parameters: fit_2gau_lin_n132d_cti_qdp,'cti_sxi_n132d_55ks_999.qdp',0.3, 0.45, 0.35, 0.4"
     return
  endif
...
;I assume each line contributes 2/3 to the total area
...
normal_guess = area_lines/(sqrt(2.*!PI)*start[2]) * 0.5

start[3] = normal_guess
pi(3).limited(0) = 1
pi(3).limited(1) = 1
pi(3).limits(0) = start[3]*0.3
pi(3).limits(1) = start[3]*1.
start[6] = normal_guess
pi(6).limited(0) = 1
pi(6).limited(1) = 1
pi(6).limits(0) = start[6]*0.5
pi(6).limits(1) = start[6]*1.5
...
print, '       Constant         En1              Sigma1          Norm1             En2           Sigma2           Norm2          Slope'
print, start
...
outfile_name = string_split[0]+'_qdp_fit_2gau_lin_cti.txt'
