...
pro fit_gau_lin_puppis_cti_qdp, file_counts_qdp, emin, emax, centroid_guess1, key_plot = key_plot

  ;;;CP, 22 Oct 2018
  ;;;
  ;;;Summary - Reads in distorted qdp spectral file (of Puppis A), fits
  ...
  if n_params() lt 4 then begin
     print,"Please enter correct parameters: fit_gau_lin_puppis_cti_qdp,filename,en_min, en_max, centroid_guess1, key_plot = 1"
     print,"Please enter correct parameters: fit_gau_lin_puppis_cti_qdp,'cti_sxi_n132d_55ks_999.qdp',1.5, 2.0, 1.8"
     return
  endif
...
if keyword_set(key_plot) then begin
   entry_device = !d.name
   plotfilenameps = string_split[0]+'_qdp_fit_gau_lin_cti.ps'
   set_plot,'ps'
   device, filename = plotfilenameps, /color,/tt_font, set_font='Times', font_size=10
   titstring = 'Puppis A fit, CTI distorted spectrum, gau + lin, file = ' + string_split[0]
