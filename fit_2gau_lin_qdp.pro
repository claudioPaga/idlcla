...
pro fit_2gau_lin_qdp, file_counts_qdp, emin, emax, centroid_guess1, centroid_guess2, key_plot = key_plot

  ;;;CP, 3 Oct 2018
  ;;;
  ;;;Summary - Reads in qdp spectral file (of N132d), fits two
  ;;;          Gaussians + BB with (A * E^2 / B^4*[exp(E/B)-1]) +
  ;;;          linear component to the brightest emission lines at low energies.
  

  
  if n_params() lt 5 then begin
     print,"Please enter correct parameters: fit_2gau_lin_qdp,filename,en_min, en_max, centroid_guess1, centroid_guess2"
     print,"Please enter correct parameters: fit_2gau_lin_qdp,'sxi_n132d_55ks_248.qdp',0.4, 0.8, 0.57, 0.66"
     return
  endif
...
;Limit linear component, it should not be negative given the shape of
;the spectrum in that range
start[7] = [0.1]
...
lin = result[0] + result[7] * en_fit
oplot, en_fit, lin, color=3
...
outfile_name = string_split[0]+'_qdp_fit_2gau_lin.txt'
...
if keyword_set(key_plot) then begin
   entry_device = !d.name
   plotfilenameps = string_split[0]+'_qdp_fit_2gau_lin.ps'
   set_plot,'ps'
   device, filename = plotfilenameps, /color,/tt_font, set_font='Times', font_size=10
   titstring = 'N132D fit, 2 gau + lin, file = ' + string_split[0]
