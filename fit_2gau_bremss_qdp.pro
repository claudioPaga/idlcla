function const_2gau_bremss, X, P

;P=[CONSTANT, CENTROID1, SIGMA1, HEIGHT1, CENTROID2, SIGMA2, HEIGHT2, SLOPE ]

y = dblarr(n_elements(X))

y = P[0] + P[3] * exp(-0.5 * ((X-P[1])/P[2])^2) +  P[6] * exp(-0.5 * ((X-P[4])/P[5])^2)  + P[7] * X^2 * exp(-X/P[8])

return, y

end

pro fit_2gau_bremss_qdp, file_counts_qdp, emin, emax, key_plot = key_plot

  ;;;CP, 18 Sept 2018
  ;;;
  ;;;Summary - Reads in qdp spectral file (of E0102), fits two
  ;;;          Gaussians + Bremm approx with (E^2 * exp(-E/kT)) to the brightest emission lines
  

  
  if n_params() lt 3 then begin
     print,"Please enter correct parameters: fit_2gau_bremss_qdp,filename,en_min, en_max"
     print,"Please enter correct parameters: fit_2gau_bremss_qdp,'smile_sxi_casa_10ks_999.qdp',0.8, 1.6"
     return
  endif
...
result = MPFITFUN('const_2gau_bremss', en_fit, counts_bin_fit, err_bin_fit, start, PARINFO=pi)
print, result
oplot, en_fit, const_2gau_bremss(en_fit, result), linestyle = 3, color = 4, thick=2

;Plot of the single gaussians

g1 = result[0] + result[3] * exp(-0.5 * ((en_fit-result[1])/result[2])^2) + result[7] * en_fit^2 * exp(-en_fit/result[8])
g2 = result[0] + result[6] * exp(-0.5 * ((en_fit-result[4])/result[5])^2) + result[7] * en_fit^2 * exp(-en_fit/result[8])
oplot, en_fit, g1, color=3
oplot, en_fit, g2, color=3

;Plot of bremss contribution

bremss = result[0] + result[7] * en_fit^2 * exp(-en_fit/result[8])

oplot, en_fit, bremss, color=4
...
outfile_name = string_split[0]+'_qdp_fit_2gau_bremss.txt'
...
if keyword_set(key_plot) then begin
   entry_device = !d.name
   plotfilenameps = string_split[0]+'_qdp_fit_2gau_bremss.ps'
   set_plot,'ps'
   device, filename = plotfilenameps, /color,/tt_font, set_font='Times', font_size=10
   titstring = 'Cas A fit, 2 gau + bremss, file = ' + string_split[0]
