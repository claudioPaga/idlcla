...
;PLOTTING START
     ...
     if n_elements(histogram_time_br) gt 1 then begin ;Makes histo if there are enough databins, otherwise just plot zeros
        ...
        xpl = indgen(fix(xrt_gti[j].stop-xrt_gti[j].start))
        ypl = intarr(n_elements(xpl))
        plot, xpl, ypl, xr=[0,xrt_gti[j].stop-xrt_gti[j].start],/ylog, yr=[0.1, 10.], xtit='time', ytit='counts', tit= '', background= 1, color=0, ystyle=8
     endelse
...
pro brte_read32, filename_evt, filename_mkf, filename_coord, g32, obsid, raobj, decobj, papnt, object, xrt_dataset32, xrt_gti_check, swift_mkf, xrt_dataset, stopper, swift_coordinates
...
pro brte_project_cla, filename_evt, filename_mkf, filename_coord, g32
...
brte_read32, filename_evt, filename_mkf, filename_coord, g32, obsid, raobj, decobj, papnt, object, xrt_dataset, xrt_gti, swift_mkf, xrt_dataset_unfilt, stopper, swift_coordinates
