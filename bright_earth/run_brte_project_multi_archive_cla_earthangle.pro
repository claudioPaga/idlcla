;10/8/12
;Andrew Spiers
pro run_brte_project_multi_archive_cla_earthangle, obsid_file, g32
...
if N_PARAMS() ne 2 then begin
    ...
    print, "Ex: IDL> run_brte_project_multi32_archive_cla_earthangle, 'grbs_obsids.txt',1"
    print, '!!!!!!!!!!!!!'
    return
endif
...
for i= 0, n_elements(obsid)-1 do begin
    ...
    sao_match =  file_search('/swift/data/archive/obs/'+obsid[i]+'/auxil/sw'+obsid[i]+'sao.fits.gz', count =count_sao)
    print,file_evt[0]
    print,mkf_match[0]
    print, coodin_match[0]
    print,  sao_match[0]
    print,''
    if strtrim(string(count_evt),2) eq '1' and strtrim(string(count_mkf),2) eq '1' and strtrim(string(count_coord),2) eq '1' and strtrim(string(count_sao),2) eq '1' then brte_project_cla_earthangle, file_evt[0], mkf_match[0], coodin_match[0], sao_match[0],  g32
                                ; kkk = get_kbrd(10)
endfor
end
